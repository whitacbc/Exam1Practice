...
def problem2a(circle, rectangle, window):
    ...
    window.render()
    window.continue_on_mouse_click()
    line = rg.Line(rectangle._upper_right_corner,rectangle.get_lower_left_corner())
    line.arrow = 'last'
...
def problem2b(rect, n, delta, win):
    ...
    for i in range(n):
        p1 = rg.Point(rect.get_upper_left_corner().x - delta*i,
                      rect.get_upper_left_corner().y - delta*i)
        p2 = rg.Point(rect.get_lower_right_corner().x + delta*i,
                      rect.get_lower_right_corner().y + delta*i)
